...
pm_apt_repos() {
    ...
    if vergt "${MINIMUM_APT_TRANSPORT_HTTPS}" "${INSTALLED_APT_TRANSPORT_HTTPS}"; then
        ...
        eval "apt -y update ${REDIRECT}" || fatal "Failed to get updates from apt.\nFailing command: ${F[C]}apt -y update"
        info "Installing APT transport for downloading via the HTTP Secure protocol (HTTPS)."
        eval "apt -y install apt-transport-https ${REDIRECT}" || fatal "Failed to install apt-transport-https from apt.\nFailing command: ${F[C]}apt -y install apt-transport-https"
    fi
    ...
    if vergt "${MINIMUM_LIBSECCOMP2}" "${INSTALLED_LIBSECCOMP2}"; then
        ...
        eval "apt -y install -t buster-backports libseccomp2 ${REDIRECT}" || fatal "Failed to install libseccomp2 from buster-backports repo.\nFailing command: ${F[C]}apt -y install -t buster-backports libseccomp2"
    fi
}
